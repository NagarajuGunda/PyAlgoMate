...
import pyalgomate.brokers.flattrade as flattrade
...
from pyalgomate.brokers.flattrade import getFutureSymbol
from NorenRestApiAsync_Flattrade.NorenRestApiAsync_Flattrade import NorenRestApiAsync_Flattrade
from NorenRestApi_Flattrade.NorenApi_Flattrade import NorenApi_Flattrade
...
def getHistoricalData(
    api: NorenRestApiAsync_Flattrade, exchangeSymbol: str, startTime: datetime.datetime, interval: str
) -> pd.DataFrame:
    startTime = startTime.replace(hour=0, minute=0, second=0, microsecond=0)
    splitStrings = exchangeSymbol.split("|")
    exchange = splitStrings[0]
    token = flattrade.getToken(exchangeSymbol)
    if "|" in token:
        token = token.split("|")[1]
...
class PaperTradingBroker(BacktestingBroker):
    ...
    def __init__(self, cash, barFeed, fee=0.0025):
        ...
        self.__apiAsync: NorenRestApiAsync_Flattrade = NorenRestApiAsync_Flattrade(
    ...
    async def retry_open_order(self, order: Order, retry_count: int):
        try:
            if retry_count == self.RETRY_COUNT - 1:
                ...
                await self.__broker.modifyFlattradeOrder(
...
class LiveBroker(broker.Broker):
    ...
    def __init__(self, api: NorenApi_Flattrade, barFeed: BaseBarFeed):
        ...
        asyncio.run_coroutine_threadsafe(
            self.__apiAsync.set_session(
                self.__api._NorenApi_Flattrade__username,
                self.__api._NorenApi_Flattrade__password,
                self.__api._NorenApi_Flattrade__susertoken,
            ),
            self.loop,
        )
    ...
    async def modifyFlattradeOrder(
        self,
        order: Order,
        newprice_type=None,
        newprice=0.0,
    ...
    async def modifyOrder(self, oldOrder: Order, newOrder: Order):
        if newOrder.isInitial():
            ...
            await self.modifyFlattradeOrder(
                order=oldOrder,
