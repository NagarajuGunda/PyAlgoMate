...
class StopLimitOrder(broker.StopLimitOrder, BacktestingOrder):
    def __init__(
        self, action, instrument, stopPrice, limitPrice, quantity, instrumentTraits
    ):
        super(StopLimitOrder, self).__init__(
            action, instrument, stopPrice, limitPrice, quantity, instrumentTraits
        )
        self.__stopHit = (
            False  # Set to true when the limit order is activated (stop price is hit)
        )
...
class Broker(broker.Broker):
    ...
    async def submitOrder(self, order):
        if order.isInitial():
            order.setSubmitted(
                ...
                f"Backtesting PyAlgoMate order {id(order)}",
            )
            self._registerOrder(order)
            # Switch from INITIAL -> SUBMITTED
            order.switchState(broker.Order.State.SUBMITTED)
            self.notifyOrderEvent(
                broker.OrderEvent(order, broker.OrderEvent.Type.SUBMITTED, None)
            )
        else:
    ...
    async def modifyOrder(self, oldOrder: broker.Order, newOrder: broker.Order):
        ...
        if newOrder.isInitial():
            newOrder.setSubmitted(
                oldOrder.getId(), self._getCurrentDateTime(), oldOrder.getRemarks()
            )
    ...
    def __preProcessOrder(self, order, bar_):
        ...
        if not order.getGoodTillCanceled():
            ...
            if expired:
                ...
                self.notifyOrderEvent(
                    broker.OrderEvent(order, broker.OrderEvent.Type.CANCELED, "Expired")
                )
    ...
    def __postProcessOrder(self, order, bar_):
        ...
        if not order.getGoodTillCanceled():
            ...
            if self.__barFeed.getFrequency() >= pyalgotrade.bar.Frequency.DAY:
                expired = (
                    bar_.getDateTime().date() >= order.getAcceptedDateTime().date()
                )
    ...
    def createMarketOrder(self, action, instrument, quantity, onClose=False):
        ...
        return MarketOrder(
            action, instrument, quantity, onClose, self.getInstrumentTraits(instrument)
        )
