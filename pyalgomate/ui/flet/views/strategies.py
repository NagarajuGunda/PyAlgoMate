...
class StrategyCard(ft.Card):
    def __init__(self, strategy: BaseOptionsGreeksStrategy, page: ft.Page):
        ...
        self.pnlText = ft.Text("₹ 0", size=25)

        self.openPositions = ft.Text("Open Pos: 0", size=12)
        self.closedPositions = ft.Text("Closed Pos: 0", size=12)
        self.balanceAvailable = ft.Text(
            f"Balance Available: ₹ {strategy.getBroker().getCash()}", size=12
        )
        ...
        if hasattr(self.strategy, "reset") and callable(self.strategy.reset):
            ...
            self.page.snack_bar = ft.SnackBar(
                ft.Row(
                    [
                        ft.Text(
                            f"Strategy {self.strategy.strategyName} has been reset.",
                            size=20,
                        )
                    ],
                    alignment="center",
            ...
            self.page.snack_bar = ft.SnackBar(
                ft.Row(
                    [
                        ft.Text(
                            f"Reset method not available for {self.strategy.strategyName}.",
                            size=20,
                        )
                    ],
                    alignment="center",
    ...
    def updateStrategy(self):
        ...
        activeBuyPositions = len(
            [
                pos
                for pos in self.strategy.getActivePositions()
                if pos.getEntryOrder() and pos.getEntryOrder().isBuy()
            ]
        )
        activeSellPositions = len(
            [
                pos
                for pos in self.strategy.getActivePositions()
        ...
        self.closedPositions.value = (
            f"Closed Pos: {len(self.strategy.getClosedPositions())}"
        )
        self.balanceAvailable.value = (
            f"Balance Available: ₹ {self.strategy.getBroker().getCash()}"
        )
        self.page.update()

    def onChartButtonClicked(self, e):
        base64Img = base64.b64encode(self.strategy.getPnLImage()).decode("utf-8")
        dlg = ft.AlertDialog(content=ft.Container(ft.Image(src_base64=base64Img)))
    ...
    def onInfoButtonClicked(self, e):
        ...
        serializableVars = {k: serialize(v) for k, v in variablesAndValues.items()}
        ...
        dlg = ft.AlertDialog(
            content=ft.Container(
                ft.Column([ft.Text(formattedText)], scroll=ft.ScrollMode.AUTO)
            )
        )
        self.page.dialog = dlg
        dlg.open = True
...
class StrategiesView(ft.View):
    def __init__(
        ...
        page: ft.Page,
        feed: BaseBarFeed,
        strategies: List[BaseOptionsGreeksStrategy],
    ):
        super().__init__()
        self.route = "/"
        ...
        self.totalMtm = ft.Text("₹ 0", size=25)
        ...
        totalMtmRow = ft.Container(
            ft.Column(
                [
                    ...
                    ft.Column([ft.Divider(color="white")]),
                    ft.Container(
                        ft.Row(
                            [
                                ft.Column(
                                ...
                                ft.Column(
                                    [
                                        ft.IconButton(
                                            ...
                                            icon_size=40,
                                            icon_color="white",
                                            on_click=self.onTotalMTMChartButtonClicked,
                                            tooltip="MTM of all Strategies running",
                                        )
                                    ]
                                ),
                            ]
                        ),
                        padding=ft.padding.only(top=35, right=10, bottom=10),
                    ),
                ]
            ),
            col={"sm": 6, "md": 2},
        ...
        self.feedText = ft.Text("Timestamps", size=10, italic=True)
        feedRow = ft.Container(
            ft.Container(
                ...
                padding=ft.padding.only(top=35, left=20, right=20, bottom=20),
            ),
            col={"sm": 6, "md": 2},
            bgcolor="#ecf0f1",
            border_radius=10,
        ...
        self.strategyCards = [
            StrategyCard(strategy, page) for strategy in self.strategies
        ]
        rows = [
            ft.ResponsiveRow(
                [totalMtmRow, feedRow], alignment=ft.MainAxisAlignment.SPACE_BETWEEN
            )
        ]
        rows.append(
            ft.ListView([ft.Row([strategyCard]) for strategyCard in self.strategyCards])
        )
        self.controls = [
            ft.AppBar(title=ft.Text("Strategies"), bgcolor=ft.colors.SURFACE_VARIANT),
            ft.Column(rows, scroll=ft.ScrollMode.HIDDEN),
        ]
    ...
    def onTotalMTMChartButtonClicked(self, e):
        ...
        fig = px.area(
            ...
            x="Date/Time",
            y=values,
            title=f"Total MTM | Current PnL:  ₹{round(pnl, 2)}",
            color=color,
        ...
        fig.add_traces(
            [
                go.Scatter(
                    x=pnlDf.query(f'strategy=="{strategy}"')["Date/Time"],
                    y=pnlDf.query(f'strategy=="{strategy}"')["PnL"],
                    mode="lines",
                    name=f"{strategy}",
                )
                for strategy in pnlDf.strategy.unique()
            ]
        )

        fig.update_layout(title_x=0.5, title_xanchor="center", yaxis_title="PnL")
        base64Img = base64.b64encode(fig.to_image(format="png")).decode("utf-8")
        dlg = ft.AlertDialog(content=ft.Container(ft.Image(src_base64=base64Img)))
        self.page.dialog = dlg
        dlg.open = True
        self.page.update()
    ...
    def updateData(self):
        ...
        totalMtm = sum([strategy.getOverallPnL() for strategy in self.strategies])
        ...
        self.feedText.value = (
            f"Quote       : {self.feed.getLastUpdatedDateTime()}\nReceived  : "
